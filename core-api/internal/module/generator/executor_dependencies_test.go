...
func TestDecodeSceneryLayoutsRejectsInvalidModelOutput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := decodeSceneryLayouts([]byte(tt.json), sceneryLayoutTestLayers(), sceneryLayoutTestDimensions())
...
func TestValidateSceneryLayoutRejectsNonFiniteValues(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*sceneryLayoutCandidate)
	}{
		{name: "position", mutate: func(candidate *sceneryLayoutCandidate) { value := math.Inf(1); candidate.Position.X = &value }},
		{name: "scale", mutate: func(candidate *sceneryLayoutCandidate) { value := math.NaN(); candidate.Scale.Y = &value }},
		{name: "rotation", mutate: func(candidate *sceneryLayoutCandidate) { value := math.Inf(-1); candidate.Rotation = &value }},
		{name: "opacity", mutate: func(candidate *sceneryLayoutCandidate) { value := math.NaN(); candidate.Opacity = &value }},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
