...
// qnaChatCompletionsAdapter calls QNA's OpenAI-compatible chat completions API.
...
// newQNAChatCompletionsAdapter creates the Chat Completions protocol adapter.
func newQNAChatCompletionsAdapter(config QNAConfig) *qnaChatCompletionsAdapter {
	...
	adapter := &qnaChatCompletionsAdapter{
		baseURL:      baseURL,
	...
	if adapter.logger != nil {
		adapter.logger.Debug("qna llm provider configured",
			logger.String("provider", qnaProviderName),
			logger.String("base_url", adapter.baseURL),
			logger.String("default_model", adapter.defaultModel),
			logger.Any("api_key_configured", adapter.apiKey != ""),
		)
	}
	return adapter
}
...
func (p *qnaChatCompletionsAdapter) Complete(
	ctx context.Context,
...
func (p *qnaChatCompletionsAdapter) completeWithResponseFormat(
	ctx context.Context,
	request *ProviderRequest,
	model string,
...
func (p *qnaChatCompletionsAdapter) logJSONObjectFallback(model string, schemaName string, err error, retry bool) {
	if p.logger == nil {
		return
	}
...
var _ LLMProvider = (*qnaChatCompletionsAdapter)(nil)
