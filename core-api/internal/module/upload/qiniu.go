...
// PutObject stores generated bytes at an explicit stable object key.
func (s *QiniuStorage) PutObject(
	ctx context.Context,
	objectKey string,
	mediaType string,
	data []byte,
	...
	if err != nil || !strings.HasPrefix(mediaType, "image/") {
		return fmt.Errorf("%w: resource media type must be an image", ErrInvalidObjectData)
	}
	return s.PutArtifact(ctx, objectKey, mediaType, data)
}

// PutArtifact stores generated bytes at an explicit stable object key.
func (s *QiniuStorage) PutArtifact(
	ctx context.Context,
	objectKey string,
	mediaType string,
	data []byte,
	...
	if err != nil || mediaType == "" {
		return fmt.Errorf("%w: valid media type is required", ErrInvalidObjectData)
	}
	if len(data) == 0 {
		return fmt.Errorf("%w: artifact data is required", ErrInvalidObjectData)
	}
...
// DeleteObject removes a generated resource that was not published in Asset
// content. Qiniu's bucket API is synchronous and does not accept a context.
func (s *QiniuStorage) DeleteObject(ctx context.Context, objectKey string) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	objectKey = strings.TrimSpace(objectKey)
	...
	if err := s.bucketManager.Delete(s.bucket, objectKey); err != nil {
		return fmt.Errorf("upload: delete object %q: %w", objectKey, err)
	}
	return nil
}
...
var _ ResourceStore = (*QiniuStorage)(nil)
