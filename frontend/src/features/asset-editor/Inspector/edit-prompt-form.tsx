...
export function EditPromptForm({
  ...
  controller: ReturnType<typeof useEditPrompt>;
  prompt: string;
        ...
        {controller.target ? (
          <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-2.5 py-2 text-xs shadow-sm">
            ...
            <Tooltip>
              <TooltipTrigger
                render={
                  <Button
                    ...
                    size="icon-xs"
                    aria-label={
                      clearSelectionTooltip ?? t("clearSelectedTarget")
                    }
              ...
              <TooltipContent>
                {clearSelectionTooltip ?? t("useEntireAsset")}
              </TooltipContent>
            </Tooltip>
          </div>
        ) : null}
...
function TargetThumbnail({ target }: { target: InspectorTargetSummary }) {
